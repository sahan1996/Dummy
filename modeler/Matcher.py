...
class Matcher:
	...
	def ransac_test(self, matches, kp1, kp2):
		...
		#fundamental, inliers = cv.findFundamentalMat(np.float32(points1), np.float32(points2), method=cv.FM_RANSAC, param1=int(self.distance), param2=self.confidence)
		fundamental, inliers = cv.findFundamentalMat(np.float32(points1), np.float32(points2), method=cv.FM_RANSAC, confidence=self.confidence)
